...
THETA = int(sys.argv[4]) # 40 -- growth rate threshold
...
class Constraint():
    ...
    def growthrate2(self,p,n):
        if n != 0:
            return (p*DN)/float((n*DP))
        elif p == 0:
            return 0
        else:
            return D
        
    def growthrate(self,p,n):
        if p >= n:
            return self.growthrate2(p,n)
        else:
            return self.growthrate2(n,p)
    ...
    def verify(self,firstValue,secondValue,orientation,items_min=None,items_max=None,transactions_min=None,transactions_max=None):      
        if self.type == 'unary':
            ...
            # emerging constraint:
            if ((firstValue == 1) and (max(self.growthrate(np.dot(DB1[:,self.var_relative_index],transactions_max), np.dot(DB0[:,self.var_relative_index],transactions_min)), self.growthrate(np.dot(DB1[:,self.var_relative_index],transactions_min), np.dot(DB0[:,self.var_relative_index],transactions_max))) >= THETA)) or (firstValue == 0):
                  return True
            else:
                return False
...
class CSP():
    def __init__(self,numItems,numTrans,setVariables,setDomains,setConstraints):
        ...
        self.max_constraint_id = numTrans+numItems*2
        self.removed_step = {}
    ...
    def printFrequentItems(self):
        # Determine frequent itemset:               
        itemset = [i for i in range(numItems) if np.max(self.domains[i].getValues()) == 1]
        
        # Number of positive classes   
        # present:
        t11 = np.sum([((np.max(self.domains[numItems+i].getValues()) == 1) and (DC[i] == 1)) for i in range(numTrans)])
        # absent:
        t12 = DP-t11
        
        # Number of negative classes
        # present:
        t21 = np.sum([((np.max(self.domains[numItems+i].getValues()) == 1) and (DC[i] == 0)) for i in range(numTrans)])
        # absent:
        t22 = DN-t21
        ...
        if t11 >= t21:
            label = 'positive'
            confidence = np.round(100*t11/float(t1),2)
        else:
            label = 'negative'
            confidence = np.round(100*t21/float(t1),2)
            
        # Compute the Chi-square test:
        xtest = np.round(self.chisqaure(t11,t21),2)
        
        # Output the result:
        print('Itemset: '+str(itemset)+' class: '+label+' confidence: '+str(confidence)+'% x^2 = '+str(xtest))
        
        # Contingency table:
        print('Active class:   present '+str(t11)+' absent '+str(t12)+' Row total '+str(DP))
        print('Inactive class: present '+str(t21)+' absent '+str(t22)+' Row total '+str(DN))
        print('Column total:   present '+str(t1)+' absent '+str(t2)+' Total '+str(numTrans))
        
    def checkSolution(self):
        # Determine associated positive transactions:
        trans_pos = [str(j)+' ' for j in range(numTrans) if ((np.max(self.domains[numItems+j].getValues()) == 1) and (DC[j] == 1))]
            
        # Determine associated negative transactions:
        trans_neg = [str(j)+' ' for j in range(numTrans) if ((np.max(self.domains[numItems+j].getValues()) == 1) and (DC[j] == 0))]

        # Verify discriminating measure:
        f_measure = self.gr(len(trans_pos),len(trans_neg))
        
        if f_measure >= THETA:
            # Determine class of pattern:
            if len(trans_pos) > len(trans_neg):
                self.num_pos_patterns+=1
            elif len(trans_neg) > len(trans_pos):
            ...
            # Print the result:
            print(''.join(itemset).strip()+' ('+str(len(trans_pos)+len(trans_neg))+': +'+str(len(trans_pos))+' -'+str(len(trans_neg))+') '+str(np.round(f_measure,2)))
    ...
    def initialization(self):
        ...
        while Q != []:
            ...
            # process emerging and closure constraints:
            elif (c.getType() == 'emerging') or (c.getType() == 'closure'):
                if self.reviseUnary(self.getDomain(c.getVariableIndex()),c,0):
                    if self.getDomain(c.getVariableIndex()).isEmpty():
                        # print explanation:
                        #!self.failureExplanation(c.getFirstVariable())
    ...
    def propagSuppress(self,Q,k):    
                ...
                if self.reviseUnary(self.getDomain(c.getVariableIndex()),c,k):
                    ...
                    if c.getVariableType() == 'T':
                        if self.getDomain(c.getVariableIndex()).getValues()[0] != 1:
                            ...
                            for i1 in freq_const_id:
                                c1 = self.constraints[i1+self.num_transactions]
                                #if (c1.getType() == 'frequency') or (c1.getType() == 'closure'):                            
                                if c1 not in Q:
                                    Q.append(c1)
                                    #if not addedFlag:
                                    #    ###print('  Constraints added:', end=' ')
        ...
        while Q != []:
            ...
            # process emerging and closure constraints:
                ...
                #!print('process emerging constraint c'+str(c.getName()))
                if self.reviseUnary(self.getDomain(c.getVariableIndex()),c,k):
                    #if self.getDomain(c.getVariableIndex()).getValues() == []:
                    if self.getDomain(c.getVariableIndex()).isEmpty():
                        # print explanation:
    ...
    def reviseUnary(self,domainX,constraint,k):
        ...
        for valueDx in domainValues:
            if not constraint.verify(valueDx,None,'forward',self.items_min,self.items_max,self.transactions_min,self.transactions_max):
                ...
                #!print('value '+str(valueDx)+' removed from domain '+constraint.getFirstVariable())
                if k > 0:
...
def loadDBsFromFiles(fileNameTr,fileNameAct):
    ...
    return dataBase,dataBase0,dataBase1,count_0,count_1,moleculeActivity,pharma_dict
...
# add emerging constraints:
for i in range(numItems):
    constraints.append(Constraint(numTrans+i+1,'emerging',variables[i]))
    
# add closure constraints:
for i in range(numItems):
    constraints.append(Constraint(numTrans+numItems+i+1,'closure',variables[i]))

# load the CSP:
task = CSP(numItems,numTrans,variables,domains,constraints)

f_out = open('results/'+fileName+'_emerging_closed+_'+str(THETA)+'.csv','w')
