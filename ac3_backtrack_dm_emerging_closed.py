...
THETA = int(sys.argv[4]) # 55 -- chi-square threshold
...
class CSP():
    def __init__(self,numItems,numTrans,setVariables,setDomains,setConstraints):
        ...
        self.max_constraint_id = numTrans+2*numItems
        self.removed_step = {}
    ...
    def reviseOriented(self,domainX,domainY,constraint,orientation,k):
        ...
        for valueDx in domainValues:
            if not self.supportExists(valueDx,domainY,constraint,orientation):
                ...
                #!print('value '+str(valueDx)+' removed from domain '+variableName)
                #!self.printDomains()
                if k > 0:
...
f_out = open('results/'+fileName+'_emerging_closed_'+str(THETA)+'.csv','w')
