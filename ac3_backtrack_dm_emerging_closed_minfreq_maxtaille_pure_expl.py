...
fileName = sys.argv[1] # 'chembl3'
fileNameTr = sys.argv[2] # 'ABL_CheEMBL_NV_MW_800_support10_258_739.csv'
fileNameAct = sys.argv[3] # 'ABL_CheEMBL_NV_MW_800_activite_molecule.txt'

THETA = int(sys.argv[4]) # 128 -- chi-square threshold
THETA2 = int(sys.argv[5]) # 8 -- max pattern size
THETA3 = int(sys.argv[6]) # 100 -- minimum frequency

patternToExplain = '|D|R||1| |D|R||3| |A|H||11| |R|R||1| |R|H||5|'
...
class Constraint():
    def __init__(self,constraintId,constraintType,firstVariable,secondVariable=None,operationType=None,unaryValue=None):
        ...
        self.var_name = firstVariable.getName()
    ...
    def verify(self,firstValue,secondValue,orientation,items_min=None,items_max=None,transactions_min=None,transactions_max=None):
        if self.type == 'unary':
            ...
            # pure solution constraint:
            if ((firstValue == 1) and (min(np.dot(DB1[:,self.var_relative_index],transactions_min), np.dot(DB0[:,self.var_relative_index],transactions_min)) == 0)) or (firstValue == 0):
                return True
            else:
                return False
...
class CSP():
    def __init__(self,numItems,numTrans,setVariables,setDomains,setConstraints):
        ...
        self.max_constraint_id = 2*numTrans+4*numItems
        self.removed_step = {}
        self.not_instantiated = []
        self.all_explanations = {}
        self.all_explanations_step = {}
    ...
    def getConstraint(self,constraintIndex):
    ...
    def checkSolution(self):      
        ...
        #if f_measure >= THETA:
        if self.chisquare(np.dot(DC,self.transactions_max),np.dot(CD,self.transactions_max)) >= THETA:
    ...
    def initialization(self):
        ...
        while Q != []:
            ...
            if (c.getType() == 'coverage') or (c.getType() == 'size'):
                if self.reviseUnary(self.getDomain(c.getVariableIndex()),c,0):
                    ...
                    if self.getDomain(c.getVariableIndex()).getValues()[0] != 1:
                        ...
                        for i1 in freq_const_id:
                            c1 = self.constraints[i1+2*self.num_transactions+3*self.num_items]
                            if 1 in self.getDomain(i1).getValues():
                                if c1 not in Q:
                                    Q.append(c1)
                                    #if not addedFlag:
            ...
            elif (c.getType() == 'frequency') or (c.getType() == 'emerging') or (c.getType() == 'closure') or (c.getType() == 'pure'):
                if self.reviseUnary(self.getDomain(c.getVariableIndex()),c,0):
                    if self.getDomain(c.getVariableIndex()).isEmpty():
                        # print explanation:
    ...
    def restoreLevel(self,k):
        while self.removed_step[k] != []:
            ...
            # remove from all_explanations what was added on k-th step:
            del self.all_explanations[v,b]
        ...
        if (k==self.num_items) or stopFlag:
            ...
            while consistent:
                ...
                if self.propagSuppress(Q,k+1) and self.searchSolutionMAC(k+1):
                    ...
                    print('Failure !!')
                    self.patternFailureExplanation(patternInTuples)
    ...
    def patternFailureExplanation(self,pattern):
        explanation = []
        for variable_id,value in pattern:
            if (self.variables[variable_id],value) in self.all_explanations.keys():
                # memorise explanations:
                explanation.append((variable_id,value,self.all_explanations[self.variables[variable_id],value]))
        if explanation != []:
            self.failure_explanations.append(explanation)
    ...
    def reviseUnary(self,domainX,constraint,k):
        ...
        for valueDx in domainValues:
            if not constraint.verify(valueDx,None,'forward',self.items_min,self.items_max,self.transactions_min,self.transactions_max):
                ...
                eliminated = True
                self.all_explanations[constraint.getFirstVariable(),valueDx] = [constraint.getName()]
...
def patternToDomains(pattern):
    # convert pattern to tuples [(0,0), (1,1), (2,0), ...]
    item_indexes = [list(PHARMA.values()).index(item) for item in pattern.split(' ')]
    
    return [[int(i in item_indexes)] for i in range(numItems)]
...
domains_list = patternToDomains(patternToExplain)

# add item domains:
for i in range(numItems):
    #domains.append(Domain([1,0]))
    domains.append(Domain(domains_list[i]))
    
# add transaction domains:
for i in range(numTrans):
    domains.append(Domain([1,0]))
...
# add pure constraints:
for i in range(numItems):
    constraints.append(Constraint(2*numTrans+3*numItems+i+1,'pure',variables[i]))

# load the CSP:
task = CSP(numItems,numTrans,variables,domains,constraints)

def printConstraintAdvanced(constraintId):
    domains = task.getDomains()
    if constraintId <= numTrans:
        # print coverage constraint:
        print('coverage constraint c'+str(constraintId)+':')
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 <->', end=' ')
        indexes = [j for j in range(numItems) if (DB2[constraintId-1,j] and domains[j]==[1])]
        if len(indexes) > 1:
            for i in indexes[:-1]:
                print(PHARMA[task.getVariable(i).getIndex()]+' +', end=' ')
            print(PHARMA[task.getVariable(indexes[-1]).getIndex()]+' = 0')
        elif len(indexes) == 1:
            print(PHARMA[task.getVariable(indexes[0]).getIndex()]+' = 0')
        else:
            print('0 = 0')
    elif (constraintId > numTrans) and (constraintId <= numTrans+numItems):
        # print frequency constraint:
        print('frequency constraint c'+str(constraintId)+':')
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 ->', end=' ')
        indexes = [j for j in range(numTrans) if (DB[j,constraintId-numTrans-1] and domains[numItems+j][0])]
        if len(indexes) > 1:
            for i in indexes[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
            print(task.getVariable(indexes[-1]+numItems).getName()+' ≥ '+str(THETA3))
        elif len(indexes) == 1:
            print(task.getVariable(indexes[0]+numItems).getName()+' ≥ '+str(THETA3))
        else:
            print('0 ≥ '+str(THETA3))
    elif (constraintId > numTrans+numItems) and (constraintId <= numTrans+2*numItems):
        # print emerging constraint:
        print('emerging constraint c'+str(constraintId)+':')
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 -> f(', end=' ')
        indexes1 = [j for j in range(numTrans) if (DB1[j,constraintId-numTrans-numItems-1] and domains[numItems+j][0])]
        indexes0 = [j for j in range(numTrans) if (DB0[j,constraintId-numTrans-numItems-1] and domains[numItems+j][0])]
        # print positive transactions:
        if len(indexes1) > 1:
            for i in indexes1[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
            print(task.getVariable(indexes1[-1]+numItems).getName()+',', end=' ')
        elif len(indexes1) == 1:
            print(task.getVariable(indexes1[0]+numItems).getName()+',', end=' ')
        else:
            print('0,', end=' ')        
        ...
        if len(indexes0) > 1:
            for i in indexes0[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
            print(task.getVariable(indexes0[-1]+numItems).getName()+') ≥ '+str(THETA))
        elif len(indexes0) == 1:
            print(task.getVariable(indexes0[0]+numItems).getName()+') ≥ '+str(THETA))
        else:
            print('0) ≥ '+str(THETA))
    elif (constraintId > numTrans+2*numItems) and (constraintId <= numTrans+3*numItems):
        # print closure constraint:
        print('closure constraint c'+str(constraintId)+':')
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 <->', end=' ')
        indexes = [j for j in range(numTrans) if (DB2[j,constraintId-numTrans-2*numItems-1] and domains[numItems+j][0])]
        if len(indexes) > 1:
            for i in indexes[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
            print(task.getVariable(indexes[-1]+numItems).getName()+' = 0')
        elif len(indexes) == 1:
            print(task.getVariable(indexes[0]+numItems).getName()+' = 0')
        else:
            print('0 = 0')
    elif (constraintId > numTrans+3*numItems) and (constraintId <= 2*numTrans+3*numItems):
        # print size constraint:
        print('size constraint c'+str(constraintId)+':')    
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 ->', end=' ')
        indexes = [j for j in range(numItems) if (DB[constraintId-numTrans-3*numItems-1,j] and domains[j][0])]
        if len(indexes) > 1:
            for i in indexes[:-1]:
                print(task.getVariable(i).getName()+' +', end=' ')
            print(task.getVariable(indexes[-1]).getName()+' ≤ '+str(THETA2))
        elif len(indexes) == 1:
            print(task.getVariable(indexes[0]+numItems).getName()+' ≤ '+str(THETA2))
        else:
            print('0 ≤ '+str(THETA2))
    elif (constraintId > 2*numTrans+3*numItems) and (constraintId <= 2*numTrans+4*numItems):
        # print pure constraint:
        print('pure constraint c'+str(constraintId)+':')       
        print(task.getConstraint(constraintId-1).getVariableName()+' = 1 -> min(', end=' ')
        indexes1 = [j for j in range(numTrans) if (DB1[j,constraintId-2*numTrans-3*numItems-1] and domains[numItems+j][0])]
        indexes0 = [j for j in range(numTrans) if (DB0[j,constraintId-2*numTrans-3*numItems-1] and domains[numItems+j][0])]
        # print positive transactions:
        if len(indexes1) > 1:
            for i in indexes1[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
        ...
        if len(indexes0) > 1:
            for i in indexes0[:-1]:
                print(task.getVariable(i+numItems).getName()+' +', end=' ')
            print(task.getVariable(indexes0[-1]+numItems).getName()+' ) == 0')
        elif len(indexes0) == 1:
            print(task.getVariable(indexes0[0]+numItems).getName()+' ) == 0')
        else:
            print('0) == 0')

def patternToTuples(pattern):
    # convert pattern to tuples [(0,0), (1,1), (2,0), ...]
    item_indexes = [list(PHARMA.values()).index(item) for item in pattern.split(' ')]
    
    return [(i,int(i in item_indexes)) for i in range(numItems)]

# Define pattern to explain:
patternInTuples = patternToTuples(patternToExplain)
...
#%%
# parse all_explanations:
all_expl_pharma = {}

for variable,value in task.all_explanations.keys():
    var_index = variable.getIndex()
    if var_index < numItems:
        all_expl_pharma[PHARMA[var_index],value] = task.all_explanations[variable,value]        

# explain purity of an element:
def getPureConstraint(item_pharma):
    item_index = list(PHARMA.values()).index(item_pharma)
    constraint_id = 2*numTrans+3*numItems+item_index+1
    
    return constraint_id

def explainPurity(element):
    constraintId = getPureConstraint(element)
    domains = task.getDomains()
    indexes1 = [j for j in range(numTrans) if (DB1[j,constraintId-2*numTrans-3*numItems-1] and domains[numItems+j][0])]
    indexes0 = [j for j in range(numTrans) if (DB0[j,constraintId-2*numTrans-3*numItems-1] and domains[numItems+j][0])]
    if len(indexes1) == 0:
        indexes = [j for j in range(numTrans) if DB1[j,constraintId-2*numTrans-3*numItems-1]]
    elif len(indexes0) == 0:
        indexes = [j for j in range(numTrans) if DB0[j,constraintId-2*numTrans-3*numItems-1]]
    
    purity_expl = {}
    for item_index in indexes:
        variable = task.getVariable(numItems+item_index)
        if (variable,1) in task.all_explanations:
            purity_expl['T'+str(item_index+1),1] = task.all_explanations[variable,1]
        else:
            print('Auchtung!')
    return purity_expl
...
itemToExplain = '|A|R||5|'
purity_expl = explainPurity(itemToExplain)
print(itemToExplain,'is pure because of the following constraints:')
print(purity_expl.values())

def constraintType(constraintId):
    if constraintId <= numTrans:
        print('c'+str(constraintId)+' is coverage')
    elif (constraintId > numTrans) and (constraintId <= numTrans+numItems):
        print('c'+str(constraintId)+' is frequency')
    elif (constraintId > numTrans+numItems) and (constraintId <= numTrans+2*numItems):
        print('c'+str(constraintId)+' is emerging')
    elif (constraintId > numTrans+2*numItems) and (constraintId <= numTrans+3*numItems):
        print('c'+str(constraintId)+' is closure')
    elif (constraintId > numTrans+3*numItems) and (constraintId <= 2*numTrans+3*numItems):
        print('c'+str(constraintId)+' is size')
    elif (constraintId > 2*numTrans+3*numItems) and (constraintId <= 2*numTrans+4*numItems):
        print('c'+str(constraintId)+' is pure')
    elif constraintId > 2*numTrans+4*numItems:
        print('c'+str(constraintId)+' is unary')
        
# print the type of each constraint:
for var_name,value in purity_expl:
    constraintType(purity_expl[var_name,value][0])

# explain each of constraints:
for var_name,value in purity_expl:
    printConstraintAdvanced(purity_expl[var_name,value][0])
#%%
# verification of constant constraints:
def checkConstraint(constraintId):
    if constraintId <= numTrans:
        # check coverage constraint:
        indexes = [j for j in range(numItems) if DB2[constraintId-1,j]]
        if len(indexes) == 0:
            print('c'+str(constraintId)+' is always true (coverage constraint)')
    elif (constraintId > numTrans) and (constraintId <= numTrans+numItems):
        # check frequency constraint:
        indexes = [j for j in range(numTrans) if DB[j,constraintId-numTrans-1]]
        if len(indexes) < THETA3:
            print('c'+str(constraintId)+' is always false (frequency constraint)')
    elif (constraintId > numTrans+numItems) and (constraintId <= numTrans+2*numItems):
        # check emerging constraint:
        indexes_pos = [j for j in range(numTrans) if DB1[j,constraintId-numTrans-numItems-1]]
        indexes_neg = [j for j in range(numTrans) if DB0[j,constraintId-numTrans-numItems-1]]
        if max(task.chisquare(len(indexes_pos),0),task.chisquare(0,len(indexes_neg))) < THETA:
            print('c'+str(constraintId)+' is always false (emerging constraint)')
    elif (constraintId > numTrans+2*numItems) and (constraintId <= numTrans+3*numItems):
        # check closure constraint:
        indexes = [j for j in range(numTrans) if DB2[j,constraintId-numTrans-2*numItems-1]]
        if len(indexes) == 0:
            print('c'+str(constraintId)+' is always true (closure constraint)')
    elif (constraintId > numTrans+3*numItems) and (constraintId <= 2*numTrans+3*numItems):
        # check size constraint:
        indexes = [j for j in range(numItems) if DB[constraintId-numTrans-3*numItems-1,j]]
        if len(indexes) <= THETA2:
            print('c'+str(constraintId)+' is always true (size constraint)')
    elif (constraintId > 2*numTrans+3*numItems) and (constraintId <= 2*numTrans+4*numItems):
        # check pure constraint:
        indexes1 = [j for j in range(numTrans) if DB1[j,constraintId-2*numTrans-3*numItems-1]]
        indexes0 = [j for j in range(numTrans) if DB0[j,constraintId-2*numTrans-3*numItems-1]]
        if (len(indexes0) == 0) or (len(indexes1) == 0):
            print('c'+str(constraintId)+' is always true (pure constraint)')

# check which constraints are always true or always false:
for i in range(2*numTrans+4*numItems):
    checkConstraint(i+1)
#%%
def explainConstraint(constraintId):
    if constraintId <= numTrans:
        # check coverage constraint:
        indexes = [j for j in range(numItems) if DB2[constraintId-1,j]]
        if len(indexes) == 0:
            print('the pattern must be covered by '+task.getConstraint(constraintId-1).getVariableName())
    elif (constraintId > numTrans) and (constraintId <= numTrans+numItems):
        # check frequency constraint:
        indexes = [j for j in range(numTrans) if DB[j,constraintId-numTrans-1]]
        if len(indexes) < THETA3:
            print('if there is '+task.getConstraint(constraintId-1).getVariableName()+' in the pattern it is always not frequent')
    elif (constraintId > numTrans+numItems) and (constraintId <= numTrans+2*numItems):
        # check emerging constraint:
        indexes_pos = [j for j in range(numTrans) if DB1[j,constraintId-numTrans-numItems-1]]
        indexes_neg = [j for j in range(numTrans) if DB0[j,constraintId-numTrans-numItems-1]]
        f_measure_max = max(task.chisquare(len(indexes_pos),0),task.chisquare(0,len(indexes_neg)))
        if f_measure_max < THETA:
            print('if there is '+task.getConstraint(constraintId-1).getVariableName()+' in the pattern it is always not emerging')
            if f_measure_max == 0:
                print('the sum of transactions is equal to the size of dataset')
    elif (constraintId > numTrans+2*numItems) and (constraintId <= numTrans+3*numItems):
        # check closure constraint:
        indexes = [j for j in range(numTrans) if DB2[j,constraintId-numTrans-2*numItems-1]]
        if len(indexes) == 0:
            print('there must be '+task.getConstraint(constraintId-1).getVariableName()+' in the pattern')
    elif (constraintId > numTrans+3*numItems) and (constraintId <= 2*numTrans+3*numItems):
        # check size constraint:
        indexes = [j for j in range(numItems) if DB[constraintId-numTrans-3*numItems-1,j]]
        if len(indexes) <= THETA2:
            print('if the pattern is covered by '+task.getConstraint(constraintId-1).getVariableName()+' its size is always ≤ '+str(THETA2))
    elif (constraintId > 2*numTrans+3*numItems) and (constraintId <= 2*numTrans+4*numItems):
        # check pure constraint:
        indexes1 = [j for j in range(numTrans) if DB1[j,constraintId-2*numTrans-3*numItems-1]]
        indexes0 = [j for j in range(numTrans) if DB0[j,constraintId-2*numTrans-3*numItems-1]]
        if len(indexes0) == 0:
            print(task.getConstraint(constraintId-1).getVariableName()+' is covered only by active transactions')
        if len(indexes1) == 0:
            print(task.getConstraint(constraintId-1).getVariableName()+' is covered only by inactive transactions')
                       
# explain all "constant" constraints:
for i in range(2*numTrans+4*numItems):
    explainConstraint(i+1)
#%%
# explanations for a chemoinformaticien:
def explainConstraintPlus(constraintId):
    if constraintId <= numTrans:
        # check coverage constraint:
        indexes = [j for j in range(numItems) if DB2[constraintId-1,j]]
        if len(indexes) == 0:
            print('the pattern must be covered by molecule #'+str(int(task.getConstraint(constraintId-1).getVariableName()[1:])-1)+' otherwise coverage constraint fails')
    elif (constraintId > numTrans) and (constraintId <= numTrans+numItems):
        # check frequency constraint:
        indexes = [j for j in range(numTrans) if DB[j,constraintId-numTrans-1]]
        if len(indexes) < THETA3:
            print('if there is '+PHARMA[task.getConstraint(constraintId-1).getVariableIndex()]+' in the pattern it is always not frequent')
    elif (constraintId > numTrans+numItems) and (constraintId <= numTrans+2*numItems):
        # check emerging constraint:
        indexes_pos = [j for j in range(numTrans) if DB1[j,constraintId-numTrans-numItems-1]]
        indexes_neg = [j for j in range(numTrans) if DB0[j,constraintId-numTrans-numItems-1]]
        f_measure_max = max(task.chisquare(len(indexes_pos),0),task.chisquare(0,len(indexes_neg)))
        if f_measure_max < THETA:
            print('if there is '+PHARMA[task.getConstraint(constraintId-1).getVariableIndex()]+' in the pattern it is always not emerging')
            if f_measure_max == 0:
                print('the sum of transactions is equal to the size of dataset')
    elif (constraintId > numTrans+2*numItems) and (constraintId <= numTrans+3*numItems):
        # check closure constraint:
        indexes = [j for j in range(numTrans) if DB2[j,constraintId-numTrans-2*numItems-1]]
        if len(indexes) == 0:
            print('there must be '+PHARMA[task.getConstraint(constraintId-1).getVariableIndex()]+' in the pattern otherwise it is not closed')
    elif (constraintId > numTrans+3*numItems) and (constraintId <= 2*numTrans+3*numItems):
        # check size constraint:
        indexes = [j for j in range(numItems) if DB[constraintId-numTrans-3*numItems-1,j]]
        if len(indexes) <= THETA2:
            print('if the pattern is covered by molecule #'+str(int(task.getConstraint(constraintId-1).getVariableName()[1:])-1)+' its size is always ≤ '+str(THETA2))
    elif (constraintId > 2*numTrans+3*numItems) and (constraintId <= 2*numTrans+4*numItems):
        # check pure constraint:
        indexes1 = [j for j in range(numTrans) if DB1[j,constraintId-2*numTrans-3*numItems-1]]
        indexes0 = [j for j in range(numTrans) if DB0[j,constraintId-2*numTrans-3*numItems-1]]
        if (len(indexes0) == 0) or (len(indexes1) == 0):
            print(PHARMA[task.getConstraint(constraintId-1).getVariableIndex()]+' is covered only by pure transactions')

# explain all "constant" constraints:
for i in range(2*numTrans+4*numItems):
    explainConstraintPlus(i+1)
#%%
